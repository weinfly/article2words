...
def browse_files(file_entry):
    file_paths = filedialog.askopenfilenames(filetypes=[('Text Files', '*.txt')], initialdir=os.getcwd())
    if file_paths:
...
def execute_function(file_entry):
    ...
    if not paths:
        messagebox.showerror('错误', '请最少选择一个txt文件，可选择多个.')
        return
...
# 创建主窗口
window = tk.Tk()
window.title('英文文章切割为单词 V1.01 支持多文件转换')
window.configure(bg='sky blue')

# 创建文件浏览小部件
file_label = tk.Label(window, text='请选择一个或多个txt文件:', bg='sky blue')
file_label.pack()
